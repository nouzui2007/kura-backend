 ...
 * - 残業: 就業終了時刻が残業基準時刻を超えた場合
 * - 早上がり: 退勤時間が早上がり手当基準時刻を下回っている場合
...
export function analyzeWorkTime(
  ...
  // 2. 残業: 就業終了時刻が残業基準時刻を超えた場合
  const overtime = endHourDecimal > systemSettings.overtimeStandardHour;

  // 3. 早上がり: 退勤時間が早上がり手当基準時刻を下回っていたら true
  const earlyLeave = endHourDecimal < systemSettings.earlyLeaveStandardHour;

  // 4. 深夜残業時間: 勤務終了が深夜開始時刻を超えた場合、勤務時間と深夜帯の重複時間
